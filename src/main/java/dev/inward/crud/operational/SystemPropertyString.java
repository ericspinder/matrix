...
public enum SystemPropertyString implements Specification.SystemProperty<String, SystemPropertyString> {
    ...
    NTLM_DOMAIN("http.auth.ntlm.authority", null, "Similar to other HTTP authentication schemes, NTLM uses the java.net.Authenticator class to acquire usernames and passwords when they are needed. However, NTLM also needs the NT authority name. There are three options for specifying the authority:Do not specify it. In some environments, the authority is not actually required and the application need not specify it. The authority name can be encoded within the username by prefixing the authority name followed by a back-slash '' before the username. With this method, existing applications that use the Authenticator class do not need to be modified, so long as users are made aware that this notation must be used. If a authority name is not specified as in method 2) and the system property \"http.auth.ntlm.authority\" is defined, then the value of this property will be used as the authority name."),
    ...
    SOCKS_Proxy_Host("socksProxyHost",null,"The SOCKS host, it's username and password are acquired in the following way. First, if the application has registered a java.net.Authenticator default instance, then this will be queried with the protocol set to the string \"SOCKS5\", and the prompt set to the string \"SOCKS authentication\". If the authenticator does not return a username/password or if no authenticator is registered then the system checks for the info preferences \"java.net.socks.username\" and \"java.net.socks.password\". If these preferences do not exist, then the system property \"info.name\" is checked for a username. In this case, no password is supplied."),
