...
public enum ResourceRecordType implements Meta_I<ResourceRecordType,ResourceRecordType> {
    ...
    StartOfAuthority("SOA",6,"RFC 1035[1] and RFC 2308[11] - Start of [a zone of] authority record	Specifies authoritative information about a DNS zone, including the primary name server, the email of the domain administrator, the domain serial number, and several timers relating to refreshing the zone."),
    ...
    locationRecord("LOC",29,"RFC 1876	Location record	Specifies a geographical location associated with a domain name"),
    ...
    namingAuthorityPointer("NAPTR",35,"RFC 3403	Naming Authority Pointer	Allows regular-expression-based rewriting of domain names which can then be used as URIs, further domain names to lookups, etc."),
    keyExchangerRecord("KX",36,"RFC 2230	Key Exchanger record	Used with some cryptographic systems (not including DNSSEC) to identify a key management engine for the associated domain-name. Note that this has nothing to do with DNS Security. It is Informational status, rather than being on the IETF standards-track. It has always had limited deployment, but is still in use."),
    ...
    tlsServerCertificatePublicKey("TLSA",52,"RFC 6698	TLSA certificate association	A record for DANE. RFC 6698 defines \"The TLSA DNS resource record is used to associate a TLS server certificate or public key with the domain name where the record is found, thus forming a 'TLSA certificate association'\"."),
    mimeCert("SMIMEA",53,"RFC 8162[9]	S/MIME cert association[10]	Associates an S/MIME certificate with a domain name for sender authentication."),
    ...
    openPgpKey("OPENPGPKEY",61,"RFC 7929	OpenPGP public key record	A DNS-based Authentication of Named Entities (DANE) method for publishing and locating OpenPGP public keys in DNS for a specific email address using an OPENPGPKEY DNS resource record."),
    ...
    serviceBindingRR("SVCB",64,"IETF Draft	Service Binding	RR that improves performance for clients that need to resolve many resources to access a domain. More info in this IETF Draft by DNSOP Working group and Akamai technologies."),
    httpsBindingRR("HTTPS",65,"IETF Draft	HTTPS Binding	RR that improves performance for clients that need to resolve many resources to access a domain. More info in this IETF Draft by DNSOP Working group and Akamai technologies."),
    ...
    uniformResouceId("URI",256,"RFC 7553	Uniform Resource Identifier	Can be used for publishing mappings from hostnames to URIs."),
    certificateAuthorityAuthorization("CAA",257,"RFC 6844	Certification Authority Authorization	DNS Certification Authority Authorization, constraining acceptable CAs for a host/domain"),
    ...
    transferAll("AXFR",252,"RFC 1035[1]	Authoritative Zone Transfer	Transfer entire zone file from the primary name server to secondary name servers."),
    incrementalZoneTransfer("IXFR",251,"RFC 1996	Incremental Zone Transfer	Requests a zone transfer of the given zone but only differences from a previous serial number. This request may be ignored and a full (AXFR) sent in response if the authoritative server is unable to fulfill the request due to configuration or lack of required deltas."),
    ...
    ResourceRecordType(String label,Integer typecode,String description) {
        ...
        this.i18n = getClass().getCanonicalName();
    }
