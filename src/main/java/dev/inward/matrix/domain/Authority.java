...
public class Authority implements Comparable<Authority> {
    ...
    public Authority(Domain domain,Zone zone) {
        this.ghost = new Identity.Ghost(new Context.Ethereal(domain,zone));
    }

    public Identity.Ghost getGhost() {
        return ghost;
    }
    ...
    public int compareTo(Authority that) {
        return this.ghost.compareTo(that.ghost);
    }

    private static final Authority RootAuthority() {
        try {
            Authority rootTld = new Authority(null);
            Domain netDomain = new Domain("net", rootTld);
            Authority netTld = new Authority(netDomain);
            Domain netRootServer = new Domain("root-servers", netTld);
            Authority netRootServerAuthority = new Authority(netRootServer);
