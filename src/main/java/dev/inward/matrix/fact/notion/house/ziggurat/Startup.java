...
import dev.inward.matrix.fact.datum.creation.Creation;
...
public class Startup {
    ...
    public static void premain(String agentArgs, Instrumentation instrumentation) throws InstantiationException {
        ...
        Map<Standard<?, dev.inward.matrix.fact.notion.concept.boot.Boot,Identity.Ego, Context.Demarc>, Resource<Boot,?,?,?, dev.inward.matrix.fact.notion.concept.boot.Boot,Init,Identity.Ego, Context.Demarc,Structure, Root>> resourceMap = new ConcurrentHashMap<>();

        Resource<Boot,Instant,Creation, Envoy.NoOp, dev.inward.matrix.fact.notion.concept.boot.Boot,Init,Identity.Ego, Context.Demarc,Structure,Root> resource = new Resource();
        Boot bootLoader = new Boot(null,);
        bootLoader.preInit(dev.inward.matrix.fact.notion.concept.boot.Boot.class);
        dev.inward.matrix.fact.notion.concept.boot.Boot boot = new dev.inward.matrix.fact.notion.concept.boot.Boot(new Identity.SuperEgo<>(new Context.Demarc(new Edition("Boot",Clock.systemDefaultZone()),true)),new Startup(instrumentation));
